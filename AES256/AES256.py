...
# add round key (initial stage of algo.)
def addroundkey(block,roundKey):
	for i in range(len(block)):
		block[i] = block[i] ^ roundKey[i]
	return block
	
#Substitute bytes using sbox and inverse substitute using sboxInv
...
tr_table = 		[2, 3, 1, 1,             #table for mix column transformation  
                1, 2, 3, 1,
                1, 1, 2, 3,
                3, 1, 1, 2]

T_2 = [ 0x00, 0x02, 0x04, 0x06, 0x08, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14, 0x16, 0x18, 0x1a, 0x1c, 0x1e,
...
T_3 = [ 0x00, 0x03, 0x06, 0x05, 0x0c, 0x0f, 0x0a, 0x09, 0x18, 0x1b, 0x1e, 0x1d, 0x14, 0x17, 0x12, 0x11,
...
T_9 = [ 0x00, 0x09, 0x12, 0x1b, 0x24, 0x2d, 0x36, 0x3f, 0x48, 0x41, 0x5a, 0x53, 0x6c, 0x65, 0x7e, 0x77,
...
T_11 = [0x00, 0x0b, 0x16, 0x1d, 0x2c, 0x27, 0x3a, 0x31, 0x58, 0x53, 0x4e, 0x45, 0x74, 0x7f, 0x62, 0x69,
...
T_13 = [0x00, 0x0d, 0x1a, 0x17, 0x34, 0x39, 0x2e, 0x23, 0x68, 0x65, 0x72, 0x7f, 0x5c, 0x51, 0x46, 0x4b,
...
T_14 = [0x00, 0x0e, 0x1c, 0x12, 0x38, 0x36, 0x24, 0x2a, 0x70, 0x7e, 0x6c, 0x62, 0x48, 0x46, 0x54, 0x5a,
...
def mixColumn(column):
	col = copy(column)
	column[0] = T_2[col[0]] ^ T_3[col[1]] ^ col[2] ^ col[3]
	column[1] = col[0] ^ T_2[col[1]] ^ T_3[col[2]] ^ col[3]
	column[2] = col[0] ^ col[1] ^ T_2[col[2]] ^ T_3[col[3]]
	column[3] = T_3[col[0]] ^ col[1] ^ col[2] ^ T_2[col[3]]
	return column
...
def mixColumnInv(column):
	col = copy(column)
	column[0] = T_14[col[0]] ^ T_11[col[1]] ^ T_13[col[2]] ^ T_9[col[3]]
	column[1] = T_9[col[0]] ^ T_14[col[1]] ^ T_11[col[2]] ^ T_13[col[3]]
	column[2] = T_13[col[0]] ^ T_9[col[1]] ^ T_14[col[2]] ^ T_11[col[3]]
	column[3] = T_11[col[0]] ^ T_13[col[1]] ^ T_9[col[2]] ^ T_14[col[3]]
	return column
